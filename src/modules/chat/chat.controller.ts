  ...
  static async chat(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!tenant || !tenant.aiEnabled || !tenant.aiApiKey) {
        return ApiResponse.error(res, 'AI Chat Assistant is not enabled for this society. Please contact the administrator.', null, 400);
      }
      ...
      const executeTool = async (name: string, args: any): Promise<any> => {
        logger.info(`AI Chat: Executing tool "${name}" for tenant ${tenantId}, user ${userId}`);
        ...
        switch (name) {
          ...
          case 'get_my_flat':
            if (userRole !== 'Resident') return { error: 'Access denied: only residents can query personal flat details.' };
            if (!resident) return { error: 'No resident flat link found.' };
            const residentFlat = await prisma.resident.findFirst({
          ...
          case 'get_my_bills':
            if (userRole !== 'Resident') return { error: 'Access denied: only residents can query personal flat bills.' };
            if (!resident || !resident.flatId) return { error: 'No flat assigned to your profile.' };
            const bills = await prisma.maintenanceInvoice.findMany({
          ...
          case 'get_my_visitor_passes':
            if (userRole !== 'Resident') return { error: 'Access denied: only residents can query visitor passes.' };
            if (!resident) return { error: 'No resident profile link found.' };
            const passes = await prisma.visitor.findMany({
          ...
          case 'get_my_complaints':
            let complaintsWhereClause: any = { tenantId, deletedAt: null };
          ...
          case 'search_directory':
            ...
            const foundResidents = await prisma.resident.findMany({
              where: {
                tenantId,
                deletedAt: null,
                OR: [
            ...
            if (userRole === 'Security Guard') {
              return {
                residents: foundResidents.map((r) => ({
                  ...
                  flat: r.flat?.number || 'N/A',
                  status: r.ownershipStatus,
                  vehicles: r.flat?.vehicles?.map((v: any) => v.plateNumber) || [],
          ...
          case 'get_user_details':
            ...
            if (targetType === 'Resident') {
              const resDetails = await prisma.resident.findFirst({
              ...
              if (!resDetails) return { error: 'Resident not found.' };
              ...
              if (userRole === 'Security Guard') {
                return {
                  ...
                  vehicles: resDetails.flat?.vehicles?.map((v: any) => ({ plate: v.plateNumber, model: v.model })) || [],
              ...
              if (!staffDetails) return { error: 'Staff member not found.' };
          ...
          case 'verify_vehicle':
            ...
            return vehicle ? {
              owner: vehicle.flat?.residents?.map((r: any) => `${r.user?.firstName || ''} ${r.user?.lastName || ''}`).join(', ') || 'N/A',
          ...
          case 'get_notices':
            const noticesList = await prisma.notice.findMany({
              where: { tenantId, deletedAt: null },
...
If a tool returns "Access denied", explain this polite policy limitation. Today's date is: ${new Date().toLocaleDateString()}.`;
      ...
      if (provider === 'GEMINI') {
        ...
        while (turn < 3 && !complete) {
          turn++;
          logger.info(`AI Chat: Gemini API POST Turn ${turn}`);
        ...
        while (turn < 3 && !complete) {
          turn++;
          logger.info(`AI Chat: OpenAI API POST Turn ${turn}`);
      ...
      return ApiResponse.success(res, { response: finalMessage }, 'Assistant response compiled successfully');
    } catch (error: any) {
      logger.error(`AI Chat completion failed: ${error.message}`);
      return ApiResponse.error(res, error.message || 'Failed to communicate with AI provider. Check your API key settings.', null, 500);
    }
  }
}
